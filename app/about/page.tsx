...
const team = [
  { initials: "EO", name: "Chief Executive Officer", role: "Founder & CEO", tint: "from-forest to-teal" },
  { initials: "CT", name: "Technology Lead", role: "Engineering & Product", tint: "from-charcoal to-teal" },
  { initials: "OP", name: "Operations Lead", role: "Lusaka & Ndola", tint: "from-teal to-leaf" },
  { initials: "CM", name: "Community Lead", role: "Volunteers & Partners", tint: "from-teal to-forest" },
];
...
export default function AboutPage() {
  return (
    <>
      ...
      <section className="bg-surface py-24 sm:py-28">
        <div className="mx-auto grid w-full max-w-7xl gap-8 px-5 sm:px-8 lg:grid-cols-2">
          <Reveal direction="left">
            <div className="group relative h-full overflow-hidden rounded-[2px] bg-gradient-to-br from-forest to-forest-deep p-9 text-white shadow-lift transition-transform duration-500 hover:-translate-y-2 sm:p-11">
              ...
              <span className="relative inline-flex h-14 w-14 items-center justify-center rounded-[2px] bg-white/15 ring-1 ring-white/25">
          ...
          <Reveal direction="right">
            <div className="group relative h-full overflow-hidden rounded-[2px] bg-gradient-to-br from-charcoal via-forest-deep to-teal p-9 text-white shadow-lift transition-transform duration-500 hover:-translate-y-2 sm:p-11">
              <div className="bg-grid-dark absolute inset-0 opacity-70" aria-hidden />
          ...
          <div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {values.map((v, i) => (
              <Reveal key={v.title} delay={0.08 * i}>
                <div className="group h-full rounded-[2px] border border-ink/10 bg-surface p-7 text-center transition-all duration-300 hover:-translate-y-1.5 hover:border-ink/25 hover:shadow-lift">
                  <span className="mx-auto flex h-13 w-13 items-center justify-center rounded-[2px] bg-brand-mint text-forest shadow-soft transition-transform duration-300 group-hover:scale-110">
      ...
      {/* Timeline */}
      <section className="relative overflow-hidden bg-paper-soft py-24 sm:py-28">
        ...
        <div className="mx-auto w-full max-w-5xl px-5 sm:px-8">
          ...
          <div className="relative mt-16">
            {/* Vertical line */}
            <div className="absolute bottom-0 left-5 top-0 w-px bg-gradient-to-b from-forest via-teal to-lime sm:left-1/2" aria-hidden />
                ...
                return (
                  <Reveal key={m.year} direction={left ? "right" : "left"}>
                    <div className={`relative flex sm:w-1/2 ${left ? "sm:pr-14" : "sm:ml-auto sm:pl-14"} pl-14 sm:pl-0 ${left ? "sm:pl-0" : ""}`}>
                      ...
                      >
                        <span className="absolute h-3.5 w-3.5 animate-ping rounded-full bg-forest/40" />
                        <span className="relative h-3.5 w-3.5 rounded-full bg-lime ring-4 ring-paper" />
                      </span>

                      <div className={`${left ? "sm:ml-auto" : ""} w-full rounded-[2px] bg-surface p-7 shadow-soft ring-1 ring-black/5 dark:ring-white/10 transition-all duration-300 hover:-translate-y-1 hover:shadow-lift`}>
                        <span className="font-mono text-sm font-bold tracking-widest text-forest">
                          {m.year}
                        </span>
          ...
          <div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-5">
            {team.map((t, i) => (
              <Reveal key={t.name} delay={0.08 * i}>
                <div className="group h-full rounded-[2px] border border-ink/10 bg-surface p-7 text-center transition-all duration-300 hover:-translate-y-1.5 hover:border-ink/25 hover:shadow-lift">
                  <span className={`mx-auto flex h-20 w-20 items-center justify-center rounded-full bg-gradient-to-br ${t.tint} font-display text-2xl font-bold text-white shadow-soft ring-4 ring-paper transition-transform duration-300 group-hover:scale-110`}>
                    {t.initials}
                  </span>
            ...
            {/* You card */}
            <Reveal delay={0.32}>
              <div className="group flex h-full flex-col items-center justify-center rounded-[2px] border-2 border-dashed border-ink/30 bg-brand-mint/50 p-7 text-center transition-all duration-300 hover:-translate-y-1.5 hover:border-lime">
                <span className="flex h-20 w-20 items-center justify-center rounded-full bg-brand-mint text-forest shadow-soft transition-transform duration-300 group-hover:scale-110">
                  <Users className="h-9 w-9" />
                </span>
                <h3 className="mt-5 font-display text-[17px] font-bold text-brand-ink">You</h3>
                <p className="mt-1 text-sm text-brand-ink/55">
                  Team members, volunteers &amp; partners — <span className="font-semibold text-forest">join us</span>.
                </p>
                ...
                <div className="overflow-hidden rounded-[2px] bg-surface shadow-soft ring-1 ring-black/5 dark:ring-white/10 transition-all duration-300 hover:shadow-lift">
                  ...
                  <div className="flex items-center justify-between gap-4 p-6">
                    <div className="flex items-center gap-3">
                      <span className="flex h-11 w-11 items-center justify-center rounded-[2px] bg-brand-mint text-forest">
